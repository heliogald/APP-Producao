...
export const getItensId = (req, res) => {
  const { id } = req.params; // Acessar o ID passado como parâmetro na rota
  const q = "SELECT *, DATE_FORMAT(dataDeLiberacao, '%d/%m/%Y') AS dataDeLiberacao FROM itens WHERE id = ?;";
...
export const addItens = (req, res) => {
  const itens = req.body.pecas.map(peca => [
    req.body.nomeCliente,
    req.body.equipamentoModelo,
    req.body.numeroDeSerie,
    req.body.dataDeLiberacao,
  ...
  const q = "INSERT INTO itens(`nomeCliente`, `equipamentoModelo`, `numeroDeSerie`, `dataDeLiberacao`, `codigo`, `descricao`, `quantidade`, `void`) VALUES ?";
...
export const updateItens = (req, res) => {
  ...
  const formattedDate = dayjs(req.body.dataDeLiberacao, 'DD/MM/YYYY').format('YYYY-MM-DD');

  const q = "UPDATE itens SET `nomeCliente` = ?, `equipamentoModelo` = ?, `numeroDeSerie` = ?, `dataDeLiberacao` = ?,\
